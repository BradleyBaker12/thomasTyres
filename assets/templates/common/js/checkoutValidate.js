        ...
        success: function (strResult) {
            ...
            if (isStripeNewCheckout() != '') {
                if (strResult.startsWith("{")) {
                    ...
                    if (jsonResult.hasOwnProperty('requiresAction')) {
                        if (jsonResult.requiresAction) {
...
function initpage() {
    ...
    LoadSaveShippingAddressPersistence();
...
function check_address2(type) {
    ...
    if (((szip.length >= 1 && sstate.length >= 1 && scountry.length >= 1) || (scountry != 'US' && scountry.length >= 1)) && addrchange == 1 && ajax_request_progress == 0) {
        ...
        if (useAddressValidator == "1" && myCountry == "US" && currentAddress != getCookie(`addressValidator-${type}`) && !isI4GoWallet()) {
        ...
        params += '&i4gowallet=' + isI4GoWallet();
        ...
        jQuery.ajax({
            ...
            success: function (strResult) {
                ...
                if (_3dThemeType == 'core' && typeof (addCarrierText) != 'undefined') addCarrierText();

                try {
                    if (jQuery("#shipResult")[0] == undefined && !isI4GoWallet()){
                        select_shipping(0)
                    }
                } catch (e) {
                    //
...
function isI4GoWallet() {
    return ((jQuery("#shift4_i4go_applepaytoken").val() != undefined && jQuery("#shift4_i4go_applepaytoken").val() != "") || (jQuery("#shift4_i4go_googlepaytoken").val() != undefined && jQuery("#shift4_i4go_googlepaytoken").val() != ""))
}
...
function LoadSaveShippingAddressPersistence(){

    if (getCookie("saveShippingAddress") == "" || getCookie("saveShippingAddress") == "true"){
        setCookie("saveShippingAddress", true, 1)
        jQuery('#shipping_saveShippingAddress').prop('checked', true);
    }

}

function PersistShippingAddressLocally(){
    
    var saveShippingAddress = jQuery('#shipping_saveShippingAddress').prop('checked');
